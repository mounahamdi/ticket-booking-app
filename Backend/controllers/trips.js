...
const getTrips = async (req, res) => {
  ...
  trips.length
    ? res.status(200).json({ message: "here are trips found", trips })
    : res.status(400).send("no trips found");
};

const addNewTrip = async (req, res) => {
  const { departure, destination, date, price, transid } = req.body;

  const existingTrip = await Trip.findOne({
    where: { transid: transid, date: date },
  });
  if (!existingTrip) {
...
const deleteTrip = async (req, res) => {
  ...
  if (tripToCancel) {
    ...
    res
      .status(200)
      .json({ message: "trip cancelled successfully", cancelledTrip });
  } else {
